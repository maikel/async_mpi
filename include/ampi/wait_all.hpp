  ...
  namespace _wait_all
  {
      ...
      void start() & noexcept {
        ...
        while (remaining > 0) {
          ...
          if (errcode != MPI_SUCCESS) {
            unifex::set_error(receiver_, errcode);
            return;
          }
          unifex::set_next(receiver_, index);
          remaining -= 1;
        }
        unifex::set_value(receiver_);
      }
    ...
    private:
      ...
      friend auto tag_invoke(
          unifex::tag_t<unifex::connect>, _sender sender, R&& recv) noexcept {
        return op<R>(
            std::move(recv),
    ...
    inline constexpr struct _fn {
      auto operator()(MPI_Comm comm, std::vector<MPI_Request> requests, int tag)
          const noexcept {
